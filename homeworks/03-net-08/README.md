...
Маршруты до публичного IPv6 адреса ниже. По непонятным причинам маршрутизатор не ищет bgp маршурт для IPv6 хоста и требует указание маски.
Логично , что при указании маски 128 префикса нет в таблице маршрутизации. Соответственно, был введен поиск маршурта для префикса /38, который провайдер анонсирует наверх (префикс был найден на ripe.net, route6 обьект).
...
Создан интерфейс loop0 с IP 1.1.1.1/32, созданы два марршрута для подсетей 2.2.2.0/24 и 3.3.3.0/24. Список команд и конфиг ниже.
