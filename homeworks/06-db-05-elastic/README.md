...
```
Проверяем состояние кластера. Состояние кластера "YELLOW" в связи с тем, что необходимы дополнительные ноды для создаения реплик, которые мы затребовали при создаении индексов. Соответсвенно для GREEN статус требуется добавить дополнительно 2 ноды при сохранении кол-ва реплик у ind-3.
```
vagrant@server1:~$ curl -X GET "localhost:9200/_cluster/health?pretty"
{
  "cluster_name" : "elasticsearch",
